...
if (isset($_POST['submit'])) {
    ...
    if ($num > 0) {
        ...
        if ($statusRow['status'] == 0) {
            ...
            header("location: http://localhost/amit_php/admin/admin_user.php");
            exit();
        }
    }
    ...
    if (empty($referral)) {
        ...
        $insert = "INSERT INTO Registration(user_id, names, email, mobile, pass, dob, referral, total_amt,`status`,`created_by`,level1,level2,level3,level4,level5,level6,level7,level8) VALUES('" . $sponsorUserId . "','" . $name . "','" . $email . "','" . $mobile . "','" . $pass . "','" . $dob . "','" . $generateReferralCode . "',$signupBonus,'" . $setStatus . "','Admin','','','','','','','','')";
        $re = mysqli_query($conn, $insert);

        // Insert into wallet
        $insertWallet = "INSERT INTO `wallet` (`user_id`, `from`, `to`, `type`, `time`, `amount`, `create_date`, `update_date`) VALUES ('" . $sponsorUserId . "','Admin', '" . $sponsorUserId . "', 'Signup Bonus', NOW() , '$signupBonus', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP())";
        ...
        if ($numReferral > 0) {
            ...
            // Insert new user
            $insert = "INSERT INTO Registration(user_id, names, email, mobile, pass, dob, referral,joining_referral, total_amt,`status`,`created_by`,level1,level2,level3,level4,level5,level6,level7,level8) VALUES('" . $newUserId . "','" . $name . "','" . $email . "','" . $mobile . "','" . $pass . "','" . $dob . "','" . $generateReferralCode . "','" . $referral . "','" . $signupBonus . "','" . $setStatus . "','Admin','','','','','','','','')";
            $re = mysqli_query($conn, $insert);

            // Insert referral bonus transaction
            $insertReferralWallet = "INSERT INTO `wallet` (`user_id`, `from`, `to`, `type`, `time`, `amount`, `create_date`, `update_date`) VALUES ('" . $referralUserId . "','" . $newUserId . "', '" . $referralUserId . "', 'Referral Bonus', NOW(), '$referralBonus', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP())";
            ...
            function updateReferralBonus($level, $referrerUserId, $newUserId, $bonus, $conn)
            {
                ...
                // Insert into wallet
                $insertWalletQuery = "INSERT INTO `wallet` (`user_id`, `from`, `to`, `type`, `time`, `amount`, `create_date`, `update_date`) VALUES ('$referrerUserId', '$newUserId', '$referrerUserId', 'Referral Level $level Bonus', NOW(), '$bonus', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP())";
    ...
    if ($re) {
        ...
        header("location: http://http://localhost/amit_php/admin/admin_user.php");
        exit();
    }
} else {
